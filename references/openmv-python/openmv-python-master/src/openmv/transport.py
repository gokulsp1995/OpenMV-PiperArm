...
#
# This module provides the low-level transport layer with the protocol
# state machine for packet parsing and communication management.
...
class Transport:
    ...
    def __init__(self, serial, crc, seq, max_payload, timeout, event_callback, drop_rate=0.0):
        ...
        self.buf = RingBuffer(max(max_payload * 4, 4 * 1024 * 1024))
        self.pbuf = memoryview(bytearray(max_payload + Protocol.HEADER_SIZE + Protocol.CRC_SIZE))
    ...
    def _check_seq(self, sequence, expected_sequence, opcode, flags):
        ...
        return (not self.seq_enabled or
                (flags & Flags.EVENT) or
                (flags & Flags.RTX) or
                sequence == expected_sequence or
                opcode == Opcode.PROTO_SYNC)
    ...
    def log(self, seq=None, ch=None, opcode=None, flags=None, length=None, direction=None, packet=None):
        ...
        opcode_str = Opcode(opcode).name if opcode in Opcode else f"0x{opcode:02X}"
        ...
        if direction == "Drop":
            ...
            emoji = "🚫"
        elif flags & Flags.ACK:
            emoji = "✅"
        elif flags & Flags.NAK:
            emoji = "❌"
        elif direction == "Send":
            emoji = "➡️"
        else:
    ...
    def send_packet(self, opcode, channel, flags, data=None, sequence=None):
        ...
        if not self.serial or not self.serial.is_open:
            raise TimeoutException("Serial connection not open")

        sequence = self.sequence if sequence is None else sequence
        length = 0 if data is None else len(data)

        if length > self.max_payload:
            raise OMVException(f"Payload too large: {length} > {self.max_payload}")

        # Pack header without CRC first (10 bytes)
        struct.pack_into('<HBBBBH', self.pbuf, 0, Protocol.SYNC_WORD,
                         sequence, channel, flags, opcode, length)
        struct.pack_into('<H', self.pbuf, Protocol.HEADER_SIZE - 2,
                         self._crc(self.pbuf[:Protocol.HEADER_SIZE - 2], 16))

        # Pack data if present
        if length > 0:
            self.pbuf[Protocol.HEADER_SIZE:Protocol.HEADER_SIZE + length] = data
            struct.pack_into('<L', self.pbuf, Protocol.HEADER_SIZE + length, self._crc(data, 32))

        packet_size = Protocol.HEADER_SIZE + length + (Protocol.CRC_SIZE if length else 0)
    ...
    def _process(self):
        ...
        while len(self.buf) > 2:
            if self.state == State.SYNC:
                ...
                if len(self.buf) < Protocol.HEADER_SIZE:
                    return None

                # Parse header
                header = self.buf.peek(Protocol.HEADER_SIZE)
                sync, seq, chan, flags, opcode, length, crc = _hdr_struct.unpack(header[:Protocol.HEADER_SIZE])
                ...
                if length > self.max_payload:
                    ...
                    self.log(seq, chan, opcode, flags, length, "Rjct")
                    self.buf.consume(1)
                elif not self._check_crc(crc, header[:Protocol.HEADER_SIZE - 2], 16):
                    self.log(seq, chan, opcode, flags, length, "Rjct")
                    self.buf.consume(1)
                else:
                    self.state = State.PAYLOAD
                    self.plength = Protocol.HEADER_SIZE + length
                    self.plength += Protocol.CRC_SIZE if length else 0
                ...
                sync, seq, chan, flags, opcode, length, crc = _hdr_struct.unpack(packet[:Protocol.HEADER_SIZE])

                # Parse payload
                if length > 0:
                    payload = packet[Protocol.HEADER_SIZE:-Protocol.CRC_SIZE]
                    payload_crc = _crc32_struct.unpack(packet[-Protocol.CRC_SIZE:])[0]
